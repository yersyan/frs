...
import {Entity, NationalTeam} from "@/types/interfaces";
...
const NationalTeams: FC<TeamsProps> = ({entities, nationalTeams}) => {
    ...
    const handleTeamClick = (team: NationalTeam) => {
        setLeftTeams(leftTeams.filter(t => t.id !== team.id));
        setRightTeams([...rightTeams, team]);
    };

    const handleRemoveTeamClick = (team: NationalTeam) => {
    ...
    const filteredLeftTeams = selectedConfederation === 0
        ? leftTeams.filter(team => team.id <= 32)
        : leftTeams.filter(team => team.entity === selectedConfederation);

    const sortedFilteredLeftTeams = filteredLeftTeams.sort((a, b) => a.id - b.id);
            ...
            <div className="grid grid-cols-1 md:grid-cols-[auto_320px] h-full gap-4">
                ...
                <div className="bg-white shadow-lg p-2 w-full">
                    ...
                    <ul className="flex flex-wrap overflow-auto max-h-[calc(100vh-128px)] gap-1">
                        {sortedFilteredLeftTeams.map((team: NationalTeam) => (
                            <li
                                key={team.id}
                ...
                <div className="bg-gray-200 shadow-lg p-2">
                    ...
                    <ul className="grid grid-cols-1 gap-y-2 overflow-y-auto max-h-[calc(100vh-96px)] px-2">
                        {rightTeams.map((team: NationalTeam) => (
                            <li key={team.id}
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-gray-600 mt-1">Ranking: {Math.round(team.id)}</p>
                                </div>
                                <button
